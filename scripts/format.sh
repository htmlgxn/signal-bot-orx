...
# Fix two-exception cases: except Exception1, Exception2: → except (Exception1, Exception2):
find tests -name "*.py" -exec sed -i 's/except \([A-Za-z_][A-Za-z0-9_.]*\), \([A-Za-z_][A-Za-z0-9_.]*\):/except (\1, \2):/g' {} \;

# Fix three-exception cases: except Exception1, Exception2, Exception3: → except (Exception1, Exception2, Exception3):
find tests -name "*.py" -exec sed -i 's/except \([A-Za-z_][A-Za-z0-9_.]*\), \([A-Za-z_][A-Za-z0-9_.]*\), \([A-Za-z_][A-Za-z0-9_.]*\):/except (\1, \2, \3):/g' {} \;
