...
class IncomingMessage:
    ...
    transport: Literal["signal", "whatsapp", "telegram"] = "signal"
...
def parse_telegram_webhook(
    ...
    from signal_bot_orx.telegram import parse_telegram_webhook as parse_telegram

    return parse_telegram(payload, bot_username=bot_username)
...
def parse_incoming_webhook(
    ...
    *,
    ...
    if transport_hint == "signal":
        return parse_signal_webhook(payload)
    if transport_hint == "whatsapp":
        return _parse_whatsapp_webhook(payload)
    if transport_hint == "telegram":
        return parse_telegram_webhook(payload, bot_username=telegram_bot_username)

    return (
        parse_signal_webhook(payload)
        or _parse_whatsapp_webhook(payload)
        or parse_telegram_webhook(payload, bot_username=telegram_bot_username)
    )


def dedupe_key(message: IncomingMessage) -> str:
    return f"{message.transport}|{message.sender}|{message.timestamp}|{message.text.strip()}"
...
def _parse_whatsapp_webhook(payload: dict[str, Any]) -> IncomingMessage | None:
    event = _resolve_whatsapp_event(payload)
    message_data = as_dict(event.get("message"))

    sender = first_non_empty_str(
        event, "from", "sender", "fromNumber", "author"
    ) or first_non_empty_str(message_data, "from", "sender", "author")
    if sender is None:
        return None
    ...
    chat_id = first_non_empty_str(
        event, "chatId", "chat_id", "conversation", "thread"
    ) or first_non_empty_str(
        message_data, "chatId", "chat_id", "conversation", "thread"
    )
...
def _resolve_whatsapp_event(payload: dict[str, Any]) -> dict[str, Any]:
